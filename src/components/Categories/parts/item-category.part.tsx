...
export default function ItemCategory({title, active, icon}: Props) {
    return active ? <div className="flex flex-col justify-center items-center md:w-24 md:py-3 bg-secondary rounded-lg mr-5">
        <div className="p-2 md:p-0 md:w-12 md:h-12 bg-white rounded-lg flex justify-center items-center">
            <img src={icon} className="w-6 h-6" />
        </div>
        <div className="mt-1">
            <span className="text-black px-4 text-xs font-semibold">
                {title}
            </span>
        </div>
